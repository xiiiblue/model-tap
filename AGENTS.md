...
- Markdown全量备份格式版本为`2`，由`MarkdownBackupCodec`维护；正文按文件夹使用`##`分组、配置使用`###`标题，Base URL输出为可点击链接，API格式、API Key和备注使用可读Markdown，便于直接存入Obsidian。导入所需的ID和时间戳隐藏在HTML注释中，多行备注使用引用块保存；必须继续兼容版本`1`的单行JSON备份导入。全量导出包含明文API Key，界面和文件正文必须明确警告；全量导入必须先校验并二次确认，确认后完整替换文件夹、配置和测试历史。
- Markdown全量备份入口使用窗口右侧工具栏中的单个菜单按钮，菜单内提供“导入全量备份”和“导出全量备份”；侧边栏只保留新增配置和新建文件夹，不显示备份入口或齿轮“设置/关于”按钮。
- 主窗口标题栏显示`ModelTap`，不要留空。
