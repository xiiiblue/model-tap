...
- 手动模型ID使用`APIProfile.manualModelIDsRaw`按行持久化。模型列表必须允许“添加”和“添加并测试”，查询结果与手动模型去重合并；查询失败时不得遮住已有手动模型，手动模型支持在整行任意位置右键删除，“测试全部”同时覆盖查询和手动模型。切换配置时只加载该配置保存的手动模型。
